...
module.exports = class Viewport {
    ...
    ignoreAreas(areas) {
        areas.forEach((area) => {
            area = this._getIntersectionWithViewport(area);
            if (area) {
    ...
    _getIntersectionWithViewport(area) {
        ...
        const bottom = Math.min(this._getAreaBottom(this._viewport), this._getAreaBottom(area));
        const left = Math.max(this._viewport.left, area.left);
        const right = Math.min(this._getAreaRight(this._viewport), this._getAreaRight(area));
