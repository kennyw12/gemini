...
const createMatcher = (matcher, opts) => {
...
module.exports = function(suite) {
    ...
    const saveBrowsers = (matchers, opts = {}) => {
        ...
        matchers = matchers.map((matcher) => createMatcher(matcher, opts));
        const filterFunc = opts.negate ? _.every : _.some;

        suite.browsers = suite.browsers.filter((browser) => {
            return filterFunc(matchers, (match) => match(browser));
        });
        return this;
    };
